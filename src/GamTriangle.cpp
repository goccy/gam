...
std::vector<Triangle> triangulate(const QList<QPointF> &points, float resolution = 50.0f)
{
	...
	for (tpp::Delaunay::fIterator mTriIt = mDelaunay.fbegin(); mTriIt != mDelaunay.fend(); ++mTriIt) {
		...
		Vec2f mTriangle[3];
		mTriangle[0] = Vec2f(points.at(mAId).x(), points.at(mAId).y());
		mTriangle[1] = Vec2f(points.at(mBId).x(), points.at(mBId).y());
		mTriangle[2] = Vec2f(points.at(mCId).x(), points.at(mCId).y());
		...
		int32_t mCounter = 0;
		QPointF pt = points.at(0);
		Vec2f mPointA(pt.x(), pt.y());//mPoints[0];
		Vec2f mPointB;
		for (int32_t i = 1; i < (int32_t)mSize; i++) {
			pt = points.at(i);
			mPointB = Vec2f(pt.x(), pt.y());
			//mPointB = mPoints[i];
